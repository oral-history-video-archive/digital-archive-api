...
namespace DigitalArchiveAPI.Controllers
{
    ...
    [ApiController]
    public class PeopleBornThisWeekController : ControllerBase
    {
        private readonly AzureSearch azureSearch;

        public PeopleBornThisWeekController(IConfiguration configuration)
        {
        ...
        {
            var results = await azureSearch.GetPeopleBornThisWeek(
                pageSize ?? 20,
                currentPage ?? 1,
                genderFacet ?? "",
                yearFacet ?? "",
