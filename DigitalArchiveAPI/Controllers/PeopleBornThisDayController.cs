...
namespace DigitalArchiveAPI.Controllers
{
    ...
    [ApiController]
    public class PeopleBornThisDayController : ControllerBase
    {
        private readonly AzureSearch azureSearch;

        public PeopleBornThisDayController(IConfiguration configuration)
        {
        ...
        {
            var results = await azureSearch.GetPeopleBornThisDay(
                pageSize ?? 20,
                currentPage ?? 1,
                genderFacet ?? "",
                yearFacet ?? "",
